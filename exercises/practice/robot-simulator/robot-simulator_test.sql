...
UPDATE tests
   SET status = 'pass'
  FROM (SELECT property, input, result FROM "robot-simulator") AS actual
 WHERE (actual.property, actual.input, JSON(actual.result)) =
       ( tests.property,  tests.input, JSON( tests.expected));
