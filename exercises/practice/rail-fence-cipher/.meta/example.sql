      ...
      (
        WITH RECURSIVE
          gen_matrix (rowid, colid, maxrow, maxcol, fill, matrix) AS (
            VALUES
              (0, 0, rails - 1, LENGTH(msg) - 1, NULL, '[[]]')
            UNION ALL
            SELECT
              IIF(colid = maxcol, rowid + 1, rowid),
        ...
        SELECT
          JSON_OBJECT(
            ...
            (
              ...
              SELECT
                JSON_GROUP_ARRAY(PRINTF('$[%d][%d]', jkey, j.key)) to_dec
              FROM
                jrows,
                JSON_EACH(jrow) j
              WHERE
...
UPDATE tmp
SET
  fence = JSON_EXTRACT(jobj, '$.matrix'),
    ...
    SELECT
      ...
      (
        WITH
          cte AS (
            WITH RECURSIVE
              rcte (string, paths, matrix) AS (
                VALUES
                  (
                    msg,
                    IIF(property = 'encode', to_enc, to_dec),
                    fence
                  )
                UNION ALL
                SELECT
          ...
          join_row_decode (string) AS (
            SELECT
              JSON_EXTRACT(matrix, j.value) AS chr
            FROM
              cte,
              JSON_EACH(to_enc) j
          )
...
FROM
  cipher
WHERE
  "rail-fence-cipher".msg = cipher.msg
