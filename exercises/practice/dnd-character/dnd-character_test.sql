...
UPDATE tests
   SET status = 'pass'
  FROM (SELECT property, input, constitution, modifier
          FROM "dnd-character"
         WHERE property = 'modifier'
           AND input    = 'score'
       ) AS actual
 WHERE (actual.property, actual.input, actual.constitution, actual.modifier) =
       ( tests.property,  tests.input,  tests.constitution,  tests.modifier)
;
UPDATE tests
   SET status = 'pass'
...
UPDATE tests
  ...
  FROM (SELECT a.property,
               ...
               a.wisdom,
               a.charisma
          FROM "dnd-character" a
               JOIN "dnd-character" b
                   ON (a.property = b.property)
         WHERE a.property = 'character'
           AND a.input    = ''
           AND b.input    = 'random'
...
UPDATE tests
   SET message = (
         'Result for "'
         || PRINTF('property=%s, input=%s, constitution=%d',
                    tests.property,  tests.input,  tests.constitution)
         || '"' || ' is <'
         || COALESCE(actual.modifier, 'NULL')
         || '> but should be <'
         || tests.modifier
         || '>'
       )
 ...
 WHERE (actual.property, actual.input, actual.constitution) =
       ( tests.property,  tests.input,  tests.constitution)
...
UPDATE tests
   SET message = (
         ...
         || JSON_OBJECT(
             ...
             'constitution', actual.constitution,
             'intelligence', actual.intelligence,
             'wisdom'      , actual.wisdom,
             'charisma'    , actual.charisma,
             'modifier'    , actual.modifier,
             'hitpoints'   , actual.hitpoints
            )
         || '> but should be <'
         || tests.expected
         || '>'
