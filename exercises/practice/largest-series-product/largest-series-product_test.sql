...
.read ./largest-series-product.sql
.output
...
UPDATE tests
   SET status = 'pass'
  FROM (SELECT digits, span, result, error
          FROM "largest-series-product") AS actual
 WHERE (actual.digits, actual.span) = (tests.digits, tests.span)
...
UPDATE tests
   SET message = (
         'Result for "'
         || PRINTF('digits=%s and span=%s', actual.digits, actual.span)
         || '" is <'
