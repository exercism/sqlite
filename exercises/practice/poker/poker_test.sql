...
UPDATE tests
   SET status = 'pass'
  FROM (SELECT hands, result FROM poker) AS actual
 WHERE JSON(actual.hands) = JSON(tests.hands)
   AND (SELECT JSON_GROUP_ARRAY(hand)
          FROM (SELECT j.value hand
...
UPDATE tests
   SET message = (
         'Result for "' || tests.hands || '"' || ' is <'
         || COALESCE(actual.result, 'NULL') || '> but should be <'
         || tests.expected || '>'
       )
  FROM  (SELECT hands, result FROM poker) AS actual
 WHERE actual.hands = tests.hands AND tests.status = 'fail';
